...
- **Voice Responses / TTS** — Speaker toggle in chat toolbar (next to mic); browser `SpeechSynthesis` reads Claude responses aloud (free, no API key); markdown stripped before speaking; text chunked at sentence boundaries for natural pausing; smart voice picker prefers Google/Samantha/Alex voices; `localStorage` persistence; toggle in both ChatInterface and ChatPanel; `lib/tts.ts` + `hooks/useTTS.ts`; OpenAI TTS upgrade path already documented in commented Option B code (requires `OPENAI_API_KEY` + `/api/tts/route.ts`)
...
- **OpenAI TTS upgrade** *(~2 hours)* — Swap the current browser `SpeechSynthesis` for OpenAI `tts-1` (higher quality, more natural voices). Requires `OPENAI_API_KEY` + `app/api/tts/route.ts`. The Option B implementation is already in commented code at the bottom of `lib/tts.ts` — just uncomment, add the env var, and create the route. Cost: ~$0.015/1K characters (~$0.005 per typical response).
