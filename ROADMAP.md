...
- **Phase 4 hardening** *(~1 day)* — Instruction-level destructive-action confirm is already in the voice session prompt. Remaining: add `DESTRUCTIVE_TOOL_NAMES` set + a code-level guard in `ChatInterface` (confirm UI before executing irreversible tools in text chat); monitor tool-selection accuracy as the list grows toward ~170 tools and tighten descriptions or filter the realtime list if accuracy degrades.
