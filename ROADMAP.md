...
- **Debt Payoff Planner** — `lib/debt-calculator.ts` month-by-month avalanche/snowball simulation; `hooks/useDebts.ts` real-time listener on `users/{uid}/debts`; `DebtPayoffPlanner.tsx` full UI with debt cards, method toggle, reactive extra-payment calculator, per-debt payoff order; `DebtPayoffWidget` dashboard widget; Debt Payoff tab in Finance page; chat tools: `add_debt`, `update_debt_balance`, `get_debts`, `delete_debt`
