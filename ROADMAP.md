...
- **FIRE Tracker** — `lib/fire-calculator.ts` pure projection functions (calcFiNumber, calcMonthsToFi); `hooks/useFireProjection.ts` reads net worth + transactions + assumptions; `FireTracker.tsx` with progress card, collapsible assumptions editor (expected return, withdrawal rate, expense/savings overrides), reactive what-if calculator; `FireWidget` dashboard widget; FIRE tab in Finance page; assumptions stored at `users/{uid}/settings/fire`; chat tools: `get_fire_projection`, `update_fire_assumptions`

- **Spending Trend Predictions** — `lib/spending-trends.ts` projects end-of-month spend rate per budget category; `GET /api/finance/trends` authenticated endpoint; `SpendingTrendsWidget` shows at-risk categories with projected overage; `spending_trend` notification category fires day 10–25 of month when any category is projected to overspend; chat tool: `get_spending_trends` *(via trends endpoint)*

- **Debt Payoff Planner** — `lib/debt-calculator.ts` month-by-month avalanche/snowball simulation; `hooks/useDebts.ts` real-time listener on `users/{uid}/debts`; `DebtPayoffPlanner.tsx` full UI with debt cards, Avalanche/Snowball/Custom method toggle, ↑↓ manual reorder with Firestore persistence at `users/{uid}/settings/debt_payoff`, reactive extra-payment calculator, per-debt payoff order; `DebtPayoffWidget` dashboard widget; Debt Payoff tab in Finance page; chat tools: `add_debt`, `update_debt_balance`, `get_debts`, `delete_debt`, `reorder_debts`
...
✅ **OpenAI Realtime API** — Shipped. `RealtimeVoice.tsx` has the full WebSocket pipeline: PCM16 capture via ScriptProcessorNode, server VAD (threshold 0.9), streaming PCM16 playback, instant interruption support, tool output capping (4 000 chars) + pruning (keeps last 6 outputs to bound context over a long call), 11-voice picker persisted to localStorage. Session endpoint at `/api/realtime/session` builds a context snapshot in parallel with the OpenAI ephemeral-key request. Tools served via `/api/realtime/tools`. Voice sessions run on GPT-4o Realtime; text chat stays on Claude.

✅ **Voice agent full-app parity (Phases 1–3)** — Voice (and text chat) can now do everything the user can do manually. Phase 1: `CLIENT_TOOL_NAMES` + `lib/client-actions.ts` client-tool layer; `navigate_to_page` (33 routes), `open_quick_link`, `regenerate_daily_summary`, `refresh_widget` wired to `DailyBriefingWidget`. Phase 2: `useWidgetRefresh` hook; all 11 refreshable dashboard widgets wired (`daily_briefing`, `weather`, `news`, `what_matters`, `insights`, `spending_trends`, `mood`, `hydration`, `budget`, `savings`, `decision_review`); `email_agent` wired to trigger an inbox scan; `open_quick_capture` wired via `QuickCaptureModal`; `start_focus_timer` wired via `MiniFocusBar`. Phase 3: text chat parity — `/api/chat` forwards client tools via `pendingClientTools`; `ChatInterface` executes them and resumes the loop.

✅ **Promote voice-to-voice to a first-class entry point** — `VoiceFloatButton` renders `RealtimeVoice float` as a fixed bottom-right button, visible on every page (hidden only when the chat panel is open or on `/chat`). Adjusts position automatically when MiniPlayer is active.

✅ **Phase 4 hardening** — `DESTRUCTIVE_TOOL_NAMES` (25 tools) in `lib/chat-tools.ts`. `/api/chat` pauses before executing any destructive tool and returns `pendingDestructiveTools`. `ChatInterface` renders an inline warning card (Confirm / Cancel); confirmed tools resume via `confirmedDestructiveIds`, cancelled ones return "Cancelled by user." Voice already had instruction-level confirm in the session prompt. Tool-selection accuracy at ~170 tools is a watch item — tighten descriptions or filter the realtime list if accuracy degrades.

- **Real-time translation mode** *(1 day)* — Travel interpreter inside the Realtime API session with a different system prompt. One-way (English → target language) and two-way (full interpreter). 16 languages, language picker UI, accessible via `/translate [language]` skill command. Personal OS context (health profile, budget) stays available in translation mode.
...
- **AI-native review** — `users/{uid}/review_cards` collection stores book highlights, journal insights, and manual captures with interval tracking. `lib/review-cards.ts` handles queries and interval math. Cards inject into the morning briefing (Claude picks the most contextually relevant one to weave in naturally) and the context snapshot (visible to every chat session). Chat tools: `get_review_cards`, `capture_review_card`, `log_review_result`. Seed route at `POST /api/review/seed` auto-populates from existing book highlights and recent journal ai_summaries with staggered initial dates. No dedicated `/review` page — surfacing is ambient via briefing and chat rather than a separate discipline to maintain.
