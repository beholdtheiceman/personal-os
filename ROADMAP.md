...
- **Promote voice-to-voice to a first-class entry point** *(~half day)* — The `RealtimeVoice` phone button is buried in the chat composer toolbar, which feels unnatural since it doesn't write to chat. Move it to a standalone button in TopNav / MobileNav that opens a focused voice-session overlay (status indicator, voice picker, end-call button), independent of the text chat panel.
