...
- **Promote voice-to-voice to a first-class entry point** *(~half day)* — The `RealtimeVoice` phone button currently lives buried in the chat composer toolbar (both `ChatInterface` and `ChatPanel`), which feels unnatural: you click it inside the chat but it doesn't write to the chat — it just starts an audio conversation. Make voice-to-voice its own dedicated, discoverable control — a standalone button in the TopNav / MobileNav (or a floating mic/phone affordance) that opens a focused "talk to your assistant" experience independent of the text chat panel. Likely a small voice-session overlay/modal with status (listening/speaking), voice picker, and end-call button. The underlying `RealtimeVoice` session logic is already self-contained and reusable; this is mostly a placement + presentation change. Consider whether the in-chat button stays as a secondary entry or is removed once the standalone one exists.
